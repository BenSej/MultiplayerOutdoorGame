...
public class TargetVisitChecker {
    ...
    public static boolean isTargetWithinRange(final double[] latitudes, final double[] longitudes,
                                              ...
                                              final int range) {
        return LatLngUtils.distance(currentLatitude, currentLongitude,
    ...
    public static boolean checkSnakeRule(final double[] latitudes, final double[] longitudes,
        ...
        for (int i = 1; i <= currentIndex; i++) {
            LatLng firstStart = new LatLng(latitudes[path[currentIndex]], longitudes[path[currentIndex]]);
            LatLng firstEnd = new LatLng(latitudes[tryVisit], longitudes[tryVisit]);
            LatLng secondStart = new LatLng(latitudes[path[i - 1]], longitudes[path[i - 1]]);
            LatLng secondEnd = new LatLng(latitudes[path[i - 1]], longitudes[path[i]]);
            if (LineCrossDetector.linesCross(firstStart, firstEnd, secondStart, secondEnd)) {
                return false;
            }
        }
        return true;
